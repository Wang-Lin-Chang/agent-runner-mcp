...
async function taskWait(args) {
    ...
    for (;;) {
        ...
        if (Date.now() > deadline)
            return { content: [{ type: 'text', text: JSON.stringify({ done: false, reason: 'timeout', exitRaw: readMaybe(exitFile) ?? null }) }] };
        await sleep(100);
    }
}
function taskOutput(args) {
...
function taskAutopsy(args) {
    ...
    const evidence = [];
    for (const f of ['lock', 'exit.txt', 'out.log'])
        if (fs.existsSync(path.join(jobDir, f)))
            evidence.push(f);
    let deathCode, manner, verdict;
    if (code === -999) {
        deathCode = 'D-06';
        manner = 'evidence tampered by task';
        verdict = 'tampered';
    }
    else if (code === -998) {
        deathCode = 'D-02';
        manner = 'sandbox apply failed, runner failed closed';
        verdict = 'failed';
    }
    else if (code === 0) {
        deathCode = 'D-01';
        ...
        deathCode = 'D-08';
        manner = 'no exit file (crash before exit write)';
        verdict = 'failed';
    }
    else {
        deathCode = 'D-02';
...
function taskKill(args) {
    ...
    if (m === null)
        return { content: [{ type: 'text', text: JSON.stringify({ killed: false, reason: 'no valid lock' }) }] };
    try {
        process.kill(Number(m[1]), 'SIGKILL');
    }
    catch {
        return { content: [{ type: 'text', text: JSON.stringify({ killed: false, reason: 'process already dead', pid: Number(m[1]) }) }] };
    }
...
function taskAdopt(args) {
    ...
    const m = lock !== undefined ? /^(\d+):(\d+)$/.exec(lock.trim()) : null;
    if (m === null)
        return { content: [{ type: 'text', text: JSON.stringify({ state: 'no-lock', note: 'already finalized or never started' }) }] };
    ...
    if (alive && isZombie(pid))
        alive = false; // 僵尸：SIGKILL 后父进程未回收，进程表项残留（启动时间不变）——状态位判死
    if (alive) {
...
async function handle(msg) {
    if (msg.method === 'initialize') {
        ...
        const fn = { task_run: taskRun, task_wait: taskWait, task_output: taskOutput, task_autopsy: taskAutopsy, task_kill: taskKill, task_adopt: taskAdopt }[name];
        if (fn === undefined) {
            respond(msg, undefined, { code: -32601, message: `unknown tool ${name}` });
            return;
        }
        try {
            respond(msg, await fn(args));
        }
        catch (e) {
            respond(msg, undefined, { code: -32603, message: String(e.message ?? e).slice(0, 200) });
        }
    }
...
process.stdin.on('data', (chunk) => {
    ...
    for (;;) {
        ...
        if (clm !== null) {
            ...
            if (Buffer.byteLength(buf.slice(clm[0].length)) < len)
                return;
